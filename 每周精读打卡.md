...
> > 
> > 论文简述：本文提出了一个大规模的VLN数据集，基于HM3D and Gibson合成了1200+个真实图像的环境（matterport只有90个）。采用duet的baseline在离散的导航任务（R2R，REVERIE，CVDN）上，采用hamt作为baseline在连续的vln任务（R2R-CE）上取得了10%以上的成功率的提升，大幅减少了seen和unseen split上的表现gap。
> 
> >
