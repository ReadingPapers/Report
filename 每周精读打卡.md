...
>>
>> 论文简述：作者认为当前OV-MER任务存在严重的目标错位（Objective Misalignment），即训练用的Token-level loss关注字符匹配，而评测用的情感轮指标关注语义距离且不可导（这导致模型分不清字符相似但语义迥异的情感词）。于是作者设计了AffectGPT-R1，将强化学习引入情感领域的尝试。方法上，模型在输出最终情感预测前需生成一段推理过程，以辅助捕捉细粒度情感语义；同时，为防止模型通过冗长无效的推理进行 reward hacking，作者引入长度惩罚与辅助奖励，对推理质量和最终预测结果进行联合约束。实验结果表明，该方法在MER-UniBench等多个基准上取得了显著提升。
>> 
