...
> > 
> > 论文简述：针对MTL中的模型剪枝问题，以前的统一直觉是总想找到一个理想的稀疏子网络去适配所有数据，但本篇工作发现了一个更好的办法，即不要让一个子网络干所有活，而是给不同类型的数据分配各自更擅长的子网络，本质上有点像无额外参数的 MoE，这样模型的能力会被用在更合适的地方。实验也确实支持这个想法，不管是图像、语音还是视频，只要数据本身差异比较大，这种分工式的子网络通常更稳。另外，论文中有一个重要的发现，也就是模型性能突然变差往往不是因为参数少了，而是不同子网开始变得越来越像，失去各自的特长，这种现象还能被掩码相似度提前预测，不过改方法又有一个新的假设，就是依赖的数据本身能被合理划分。
> >
