...
> >
> > 论文简述：本论文提出了MMML的方法，主要就是一种多模态多损失的特征融合网络。具体网络结构如图所示，只使用T&A两种模态，然后做N层的corss_attention和self-attention，主要工作是在损失的设计上：采用单个的模态特征与融合后的特征一起做loss,但是最终预测时只使用融合后的特征进行预测。文中描述的这样的好处：1. 针对CH-SIMS数据集，不同的模态有着不同的标签，这样子做可以更好的去进行情感识别（实验结果验证有效）。 2.利用多损失策略，也可以提高单个模态子网络的性能，这可能为多模态缺失的情感识别任务提供另一种可能。 但是本文方法也有一些不足之处以及自己的看法： 1. 没有使用到视频特征（文中说是添加之后效果变差，感觉还是没有完全做好多模态融合， 根据网络结构看出融合策略还是较为简单） 2. 这种特征融合的方式以及多损失方法在CMU-MOSEI数据集上并没有提升（甚至下降），只是在CH-SIMS数据集（算是新数据集，工作较少）上有明显效果。 3. 这种端到端的多损失的方法，一个明显的问题就是loss下降的目标方向与预测的目标方向不一致，这可能是导致在CMU-MOSEI数据集上表现不好的原因，或许可以改成两个阶段进行尝试，将多loss的方法，拆开成两个阶段的loss进行训练。 4. 本文网络结构没有特别突出，但是最终结果却部分达到sota(加上context，就是sota), 可能主要原因还是因为数据的特征工程做的比较好，文中也提到了很多这方面的实验探究。
总结： 方法结构比较简单，已给出源码，后续可以进行尝试：1.加上视频模态进一步尝试出更好的模型结构 2. 实现之前预想的两阶段的loss代替端到端一阶段的多loss方法 3. 验证模型简单结果优秀的真正原因（特征提取优秀？）
>
