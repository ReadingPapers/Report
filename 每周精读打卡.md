...
> ==> 时间倒序，本次更新插入位置 <==
>
> > [2025-04-12]-[陈银]-[Bimodal Fusion Framework for Dynamic Facial Expression Recognition  in-the-wild] ICCV 2025 投稿
> >
> > <img width="512" alt="image" src="https://raw.githubusercontent.com/cyinen/imge/master/20250412124352.png">
> >
> > 论文简述: 这篇论文提出了一个叫做 BFF-DFER 的双模态融合框架，专门用来做动态面部表情识别 (DFER)，结合了音频和视觉两种模态。核心思路是基于预训练的 Transformer 模型，加入了三个模块：MSA (Modality-Specific Adaptation) 处理单一模态的特征增强，BFA (Bimodal Fusion Adapter) 负责跨模态融合，CMTM (Cross-Modal Temporal Modeling) 捕捉时序信息。在 DFEW 和 MAFW 数据集上的测试结果还不错，尤其是在 DFEW 上拿到了目前最好的 Weighted Average Recall (WAR) 78.28% 和 Unweighted Average Recall (UAR) 67.52%。
>
