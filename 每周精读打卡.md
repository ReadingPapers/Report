...
> ==> 时间倒序，本次更新插入位置 <==
>
> >[2024-1-13]-[徐赟博]-[ENVEDIT: Environment Editing for Vision-and-Language Navigation]
...
>
> > [2024-01-12]-[贾朋]-[SyncTalk- The Devil is in the Synchronization for Talking Head Synthesis](https://ziqiaopeng.github.io/synctalk/)
...
> 
> > [2024-01-12]-[张雪松]-[Robustness Analysis of Video-Language Models Against Visual and Language Perturbations](https://proceedings.neurips.cc/paper_files/paper/2022/hash/de6ff07cbd222c10d694c2b2f732aceb-Abstract-Datasets_and_Benchmarks.html)
...
> > [2024-1-12]-[聂建涛]-[Detecting Facial Action Units From Global-Local Fine-Grained Expressions]-[paper](https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=10160018)
> >
...
>
> > [2024-1-5]-[聂建涛]-[FaceMixup: Enhancing Facial Expression Recognition through Mixed Face Regularization]-[paper](https://arxiv.org/pdf/2405.20259)
> >
...
> >[2024-12-30]-[于洋晨]-未精度
>
> >[2024-12-30]-[张雪松]-改自己的论文+泛读，未精读
> 
> > [2024-12-29]-[何艺超]-本周准备三门考试+实验微调数据集处理，没有来得及精读
>
...
> >
> >  <img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/Micro-expression%20Recognition%20Based%20on%20Facial%20Graph%20Representation%20Learning%20and%20Facial%20Action%20Unit%20Fusion.png">
> >
...
> >
> ><img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/Uni-Navid.png" />
> >
...
> >
> >论文简述：提出现有SEC任务（语音情感caption生成）的不足（faithfulness hallucination，factuality hallucination以及lose generalization）提出解决方案：1.作者通过知识蒸馏的方法减少语音和文本输入之间的不一致分布，具体来说，将标记的cation先经过教师模型分析提取的语义和通过NLTK包解析的情感凝练词来作为指导，并使用KL散度来评价不同模态的分布距离，判断是否对齐。2.作者提出了人类偏好对的方法并配合以增加情感细粒度prompt来减少幻觉问题，具体来说是通过gpt3.5对一个样本的多次反馈打分，选择分数高的作为实际的人类偏好caption。其他细节：使用MER2023来生成MER23SEC数据集，在NNIME数据集评估泛化性，并提出EMOSEC1的新数据集。此任务评估方式：用gpt3.5评估情感线索和总结状态的重叠程度，总体的实验上看在caption的生成中，相比baseline增加了更多的情感描述词汇，也就是减缓了lose generalization的问题。
> >
...
> >
> >  <img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/Real-time%20attention%20monitoring%20system%20for%20classroom.png">
> >
...
> >
> > <img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/appe.png" />
> >
...
> > <img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/YoutubeVLN.png">
> >
...
> > 
> > <img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/cefl.png">
> >
...
> >
> >  <img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/Class%20activation%20regularizationen.png">
> >
...
> > [2024-11-24]-张雪松]-[SPOC: Imitating Shortest Paths in Simulation
...
> > 
> > [2024-11-24]-[于洋晨]-[Multimodal Multi-loss Fusion Network for Sentiment Analysis]-[ 2024 NAACL ](https://aclanthology.org/2024.naacl-long.197/)
> >  
...
> >论文简述：提出了一种Bottleneck tokens，旨在更好的多模态融合，在token融合前，加入临时的bottleneck token，该组token数量很少，利用该组token进行模态间的交互。此方法在后来的研究中也多有采用，是除了直接cat之外的一个比较通用且有效的token融合方法。
...
> > <img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/StyleSwin.png">
> >
...
> >
> > 论文简述：AU检测的工作貌似还停留在FER中的传统方法阶段，此工作就是通过设计网络结构进行AU检测的改进，和以往工作不同的是：将AU分区域（上，中，下），考虑区域内部和外部的激活关系。另外在时间模块，动态AU的变化可能没有FER那么大，所以帧特征两两相减，突出微妙的AU帧差异。通过这两个构造，可以巧妙的拉进AU激活区域而忽略非激活区域带来的误判，最终不借助额外数据集也到达了BP4D、DISFA的SOTA。
>
...
> > <img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/Visual%20Prompt%20Multi-Modal%20Tracking.png">
> >
...
>
> > [2024-11-02]-[于洋晨]-[Decoupled Multimodal Distilling for Emotion Recognition]-[ 2023 CVPR Paper](https://openaccess.thecvf.com/content/ICCV2023/html/Wang_Distribution-Consistent_Modal_Recovering_for_Incomplete_Multimodal_Learning_ICCV_2023_paper.html)
> >  
...
> >
> > 论文简述：通过设计的vision tokenizer，将视频、图像统一到文本空间，一起进入模型。代理任务则是自回归生成的next token prediction。方法较为简单，但效果极佳，在视频和图像的理解，生成等方面都有不错的效果。
...
> >
> ><img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/PMFER.png">
> > 
> >论文简述:
> >
...
> >
> > 论文简述：提出了一种从自然语言路径指令生成拓扑地图的方法，旨在解决基于视觉和语言导航(VLN)中用户每次导航都需要详细解释路径的问题。通过使用大语言模型（LLM），研究人员提出了两种生成地图的方式：一种是隐式存储地图，另一种是显式生成包含节点和边的拓扑地图。实验结果表明，显式地图的生成方法在路径推断和导航精度上优于隐式存储，大大降低了用户解释路径的负担。
> >
> > 
...
> ><img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/EmoLA.png">
> >
...
> > <img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/AC_POLICY.png">
> >
...
> >
> ><img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/ICCV.png">
> > 
> >论文简述:
> >
...
> 
> [2024.9.29]-[陈银]-[Fine-Grained Temporal-Enhanced Transformer for Dynamic Facial Expression Recognition]-[IEEE Signal Processing Letters]\
> >
...
> > <img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/Ig3D.png">
> >
...
> 
> [2024-9-21]-[徐赟博]-[Dynamic Task Prioritization for Multitask Learning]https://openaccess.thecvf.com/content_ECCV_2018/papers/Michelle_Guo_Focus_on_the_ECCV_2018_paper.pdf)
> ><img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/DTP.png"> 
> >
> >论文简述：本文介绍了一种针对不同难度的学习任务进行权重调节的方法-根据任务难度动态调节。因为不同任务的学习难度不同，简单任务的优化很可能不如复杂任务的重要。本文提出使用一种KPI（关键性能指标）作为监督任务学习难度的信号，KPI可以选择任何衡量任务完成进度的指标（准确度\AP）,并通过滑动平均维持KPI的稳定性。对于多任务权重的调节，作者提出了分为实例和任务两个层面，前者调节单个样本的权重，后者调节整个任务loss的权重。调节方案都是一样的，通过Focal Loss动态改变困难样本权重的方式，通过当前KPI监督在训练中动态调节不同级别的权重。在分类、分割、检测、姿态估计四个多任务设置下上作者进行了实验，和不同的多任务权重调节方式以及和单任务模型进行了对比实验，证明了方法的有效性。并且通过对训练过程中优先级的记录，发现在训练初期，姿态估计的优先级最高，但在后期检测和分类的优先级也得到了逐渐的提升。
>
> [2024-9-21]-[陈银]-[Norface: Improving Facial Expression Analysis by Identity Normalization](https://arxiv.org/abs/2407.15617)
> > <img width="512" alt="image" src="https://raw.githubusercontent.com/cyinen/imge/master/20240912194649.png"> 
> >
> > 论文简述：本文介绍了一种新的人脸表情分析方法。由于模型的性能往往受限于人脸表情图像中和表情无关的背景，姿态的变化，给表情分析带来了很大的挑战。本文提出了将身份归一化，把所有图像的表情都归一化到一个没有背景和噪声的图像上，保持表情不变，以此来去除姿势等噪声的影响。为了实现这个目的，文中使用了对抗损失、重建损失、感知损失、身份损失等多种损失来约束模型的学习。最终在 AU 检测、AU估计和表情识别等多个任务上都得到了极大的提升。
> >


> [2024-8-11]-[徐赟博]-[Layout-aware Dreamer for Embodied Referring Expression Grounding](https://arxiv.org/pdf/2212.00171)
> ><img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/LAD.jpg"> 
> >
...
> >
> 
> [2024-8-4]-[徐赟博]-[Sub-Instruction Aware Vision-and-Language Navigation(2020)](https://arxiv.org/pdf/2004.02707)
> ><img width="512" alt="image" src="https://github.com/ReadingPapers/Report/blob/main/Images/FGR2R.jpg">  
> >
...
> >
>  [2024-7-21]-[张雪松]-[Depth-Aware Vision-and-Language Navigation using Scene Query_Attention Network](https://ieeexplore.ieee.org/abstract/document/9811921)-[ICRA-2022]    
> > <img width="512" alt="image" src="https://github.com/user-attachments/assets/94e036fc-78f7-410a-8eb3-8fb9daa6917e">   
> >
> >论文简述:
> > 直接使用深度图像作为输入的dual-stream方式对模型导航的提升很小，现有新的特征（如detector region等）也是基于RGB observation，会引入语义不相干的纹理细节，使得模型过拟合访问过的环境。
> >本文试图通过通过深度信息 与RGB observations结合构建语义map（一种高水平的抽象的表示）防止模型过度拟合；另外端到端地引入并构建这种语义map也会带来更多的参数导致过拟合，所以本文预训练一个map encoder（类似detr的方式）。
> >
...
>>
> [待补充]-[张雪松]-[Diagnosing Vision-and-Language Navigation: What Really Matters](https://aclanthology.org/2022.naacl-main.438/)-[ACL-2022]    
> > <img width="512" alt="image" src="https://github.com/user-attachments/assets/fa1b2833-0c72-4126-bd92-348072e78ce3">   
> >论文简述:
>>
...
> > <img width="512" alt="image" src="https://raw.githubusercontent.com/2351548518/images/main/20240609/202406241051739.png">
...
>论文框架图：
> > <img width="512" alt="image" src="https://github.com/ReadingPapers/Report/assets/90198143/174740da-caf0-4678-b196-aaf4a192670a">
> 
> >论文简述：\
