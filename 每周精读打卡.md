<img width="1244" height="413" alt="image" src="https://github.com/user-attachments/assets/8a893bb5-6c48-455a-9888-dbf8ea02269b" />######### 模版 #############
...
>>
>> 论文简述：这篇研究探讨了VLM在FER任务中，如何依赖面部特征，牙齿可见性、眉毛位置和嘴巴开合，来预测情绪。研究发现，尽管这些模型在zeroshot情感识别任务中表现优异，但它们往往大多依赖牙齿的可见性，而这些特征并不全面。牙齿可见性特别影响高情感值的预测，但也可能导致误分类（当牙齿被错误预测为可见时，模型倾向于将情绪归类为快乐）。至少在Affectnet数据集中，可见的牙齿成为了zeroshotVLM或监督学习模型的线索，这也成为了这个数据集的bias。这种情况是由各个数据集的bias给模型提供了一些捷径，导致训练出的模型缺乏可解释性和泛化性。这篇工作只是发现了这个问题存在，没有提出解决它的办法。
> >
